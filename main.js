    ...
    static async Signup() {
        const signInData = await inquirer.prompt([
            ...
            },
            {
                ...
                validate: (input) => /^\d{11}$/.test(input) ||
        ...
        return new User(signInData.name, signInData.surName, signInData.age, signInData.gender, signInData.EmailID, signInData.password, signInData.phone, 0);
    }
...
class Admin {
    ...
    static async editEvent(events) {
        ...
        const eventDate = new Date(eventDetails.date);
        if (eventDate <= currentDate) {
            console.log(chalk.bold.redBright("Error: Event date must be in the future."));
            return;
        }
    ...
    static async purchaseTickets(events) {
        if (events.length === 0) {
...
async function main() {
    ...
    while (!logOut) {
        const { action } = await inquirer.prompt([
            {
                name: "action",
                message: chalk.bold.italic.cyanBright("Would you like to Signup, Login, or Exit?\n"),
                type: "list",
